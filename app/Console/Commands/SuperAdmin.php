...
class SuperAdmin extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $email      = !$this->option('email') ? 'admin@AfyaPacs.net' : $this->option('email');
            $password   = !$this->option('password') ? 'AfyaPacs@123' : $this->option('password');
            if ($result) {
                $this->components->error('Superadmin is already in the Database.');
                $this->line('<bg=black;fg=white>..:: Created by AfyaPacs ::..</>');
            } elseif (!$result) {
                $user = User::create([
                    'name'              => $username,
                    'email'             => $email,
